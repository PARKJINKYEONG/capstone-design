...
const DocumentDirectoryPath = RNFS.DocumentDirectoryPath;
  ...
  const ResultScreen = () => {
    ...
    const { resultString, selectedImage } = route.params;
    ...
    const handleSaveButtonPress = () => {
      navigation.navigate('SaveInput', { selectedImage, text: resultArray[0][0] });
    };
          ...
          <View style={styles.rankInfoContainer}>
            ...
            <View style={styles.inputContainer}>
              {resultArray.length > 0 && (
                <Text style={styles.input}>{resultArray[0][0]}</Text>
              )}
            </View>
          </View>
          <View style={styles.progressBarContainer}>
            <ProgressBar progress={resultArray.length > 0 ? resultArray[0][1] / 100 : 0} width={150} height={15} color={'green'} />
          ...
          <View style={styles.rankInfoContainer}>
            ...
            <View style={styles.inputContainer}>
              {resultArray.length > 1 && (
                <Text style={styles.input}>{resultArray[1][0]}</Text>
              )}
            </View>
          </View>
          <View style={styles.progressBarContainer}>
            <ProgressBar progress={resultArray.length > 1 ? resultArray[1][1] / 100 : 0} width={150} height={15} color={'blue'} />
          ...
          <View style={styles.rankInfoContainer}>
            ...
            <View style={styles.inputContainer}>
              {resultArray.length > 2 && (
                <Text style={styles.input}>{resultArray[2][0]}</Text>
              )}
            </View>
          </View>
          <View style={styles.progressBarContainer}>
            <ProgressBar progress={resultArray.length > 2 ? resultArray[2][1] / 100 : 0} width={150} height={15} color={'red'} />
